...
Lingyu Chen<sup>*</sup>, Yue Wang<sup>*</sup>, Cheng Li, Lin Jin, Daoqiang Zhang, Hongen Liao, Fang Chen, Qingjie Meng
...
Breast ultrasound is widely used for breast lesion screening, but reliable interpretation remains difficult because ultrasound images often contain speckle noise, acoustic artifacts, and ambiguous morphology. Most existing BUS classification methods optimize direct image-to-label prediction and overall accuracy, but do not explicitly follow clinical diagnostic rules or expose a traceable decision pathway.

RACA reformulates BUS classification as a structured, rule-governed decision process. It first extracts morphological and semantic evidence from the lesion region, then maps fused diagnostic evidence into a rule-aligned malignancy risk estimate, and finally refines the decision through critic-guided memory retrieval. Experiments on three public BUS benchmarks show that RACA outperforms representative classification and medical agent-based methods, while interpretability analyses demonstrate stable and clinically meaningful evidence patterns.
